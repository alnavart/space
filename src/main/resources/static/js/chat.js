var chat = {
    comms : {
        ...
        connect: function(rcvMessage, rcvEvent, email) {
            ...
            chat.comms.stompClient = Stomp.over(socket);
            chat.comms.stompClient.connect({'X-Email':email}, function(frame) {
    ...
    gui : {
        ...
        showMessage: function(email, text){
            var gravatar = chat.gui.getGravatar(email);
            var message;
            if (email != chat.gui.email) {
        ...
        showAllParticipants: function(emails){
            ...
            for (index = 0; index < emails.length; ++index) {
                ...
                chat.gui.addParticipant(emails[index]);
            }
        },
